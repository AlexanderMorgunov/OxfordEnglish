...
export async function parsePdf(bytes: Uint8Array): Promise<ParsedBook> {
  // A fresh worker per call, terminated when done. A single long-lived workerPort becomes
  // unusable after task.destroy(), which would break the second PDF import in a session.
  ...
  try {
    ...
    // Scaled with page count so a scanned book (≈0 chars) is caught but a short handout isn't.
    if (totalChars < Math.max(20, doc.numPages * 8)) throw new Error('pdf-no-text-layer');
