...
/** v8 (slice 2): sync scaffolding. Renumbered from v7 when the backend branch merged: production
 *  had already shipped v7 (activity/reviewLog), and an upgrade at or below the installed version never
 *  runs — existing installs would never get these tables. Adds sync-meta indexes + a globally-unique `&syncId` for the
