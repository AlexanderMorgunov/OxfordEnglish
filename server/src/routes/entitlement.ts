...
import { evaluate, grantTrial, installHash, indexKeyConfigured, type EntitlementStore } from '../entitlements.js';

const err = (code: string, status: 400 | 401 | 409 | 503) => Response.json({ error: { code } }, { status });
...
export function entitlementRoutes(store: EntitlementStore): Hono {
  ...
  app.post('/v1/entitlement/trial', async (c) => {
    ...
    // Minting a trial without the key would write an install marker bound to nobody.
